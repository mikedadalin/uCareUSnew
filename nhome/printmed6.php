...
<table border="1" style="border-collapse:collapse; font-size:10pt;" width="1480">
  <tr id="backtr"  style="border:none;" height="20">
    ...
    <td class="title" width="80" style="border:none;">Comment</td>
    <td width="80" style="border:none;"><?php echo draw_option("Qintake","Powdery;NG","s","multi",$Qintake,false,5); ?></td>
  </tr>
  <tr id="backtr"  style="border-top:1px solid #000;" height="20">
    <td class="title" width="60" style="border:none;">Allergic drug</td>
...
<table border="1" style="border-collapse:collapse; text-align:center; table-layout:fixed;" width="1480">
  <tr class="title" height="30">
    <td width="50">起迄日</td>
    <td width="30">Source</td>
    <td width="170">Medication</td>
    <td width="30">Time</td>
    <?php
  ...
  for ($i=0;$i<$stopno;$i++) {
	  ...
	  echo '
  <tr height="36">
	<td rowspan="'.$rowspan.'">'.formatshortdate(str_replace("/","",$r['Qstartdate'])).'<br>|<br>'.formatshortdate(str_replace("/","",$r['Qenddate'])).'</td>
    ...
    <td>'; if (count($time)>0 && $time[0]<=9) { echo $time[0]; } else { echo '&nbsp;'; } echo '</td>'.drawmedcal($bgday,$r['Qmedday']).'
  </tr>'."\n";
    if (count($time)<=4) {
		$time2 = '&nbsp;';
		$time3 = '&nbsp;';
		...
		for ($t1=0;$t1<count($time);$t1++) {
			if ($time[$t1]>9 && $time[$t1]<=13) { if ($time[$t1]>12) { $time2 = ($time[$t1]).''; } elseif ($time[$t1]==12) { $time2 = '12'; } else { $time2 = $time[$t1].''; } }
			elseif ($time[$t1]>13 && $time[$t1]<=18) { $time3 = ($time[$t1]).''; }
			elseif ($time[$t1]>18 && $time[$t1]<=23) { $time4 = ($time[$t1]).''; }
		}
		echo '
		<tr height="36"><td>'.$time2.'</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>
		<tr height="36"><td>'.$time3.'</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>
		<tr height="36"><td>'.$time4.'</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>
		';
	} elseif (count($time)==0) {
		echo '<tr height="36"><td>&nbsp;</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>'."\n";
		echo '<tr height="36"><td>&nbsp;</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>'."\n";
		echo '<tr height="36"><td>&nbsp;</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>'."\n";
	} else {
		for ($t1=0;$t1<count($time);$t1++) {
			echo '<tr height="36"><td>'.$time[$t1].'</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>'."\n";
		}
	}
...
<?php
	if ($pageno==$page) {
		echo '<p>The &nbsp;&nbsp;&nbsp;頁&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font size="2">Abbreviation: Not through mouth-NPO　Refuse - Ref　Outgoing - Out　睡眠 - Slp　Shortage of drug -A　不良反應 - O/X　Other-＊ (Noted in Nursing records)</font></p>';
	} else {
		echo '<p style="page-break-after: always;">The &nbsp;&nbsp;&nbsp;頁&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font size="2">Abbreviation: Not through mouth-NPO　Refuse - Ref　Outgoing - Out　睡眠 - Slp　Shortage of drug -A　不良反應 - O/X　Other-＊ (Noted in Nursing records)</font></p>';
	}
}
?>
</body>
