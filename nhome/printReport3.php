...
if($_GET["STK_NO1"]!=""){
	$strQry .= " AND b.STK_NO >= ".$_GET["STK_NO1"];
}
if($_GET["STK_NO2"]!=""){
	$strQry .= " AND b.STK_NO <= ".$_GET["STK_NO2"];
}
...
$db = new DB;
$dbCount = new DB;
$str = "SELECT distinct(a.firmID) FROM `".$strModule."` a inner join `".$subModule."` b on";
...
$str .=" and left(a.firmID,4)='Area' order by a.firmID";
//$str = "select areaID as firmID from arkarea ";
...
$db1 = new DB;
$str1 = "SELECT distinct(b.STK_NO) hospID FROM `".$strModule."` a inner join `".$subModule."` b on";
...
$str1 .=" and left(a.firmID,4)='Area' order by b.STK_NO";
//$str1 = "select distinct(STK_NO) hospID from arkstock where STK_KIND1='1' AND STK_KIND2='1' ";
...
for ($j=1;$j<=$patientSize;$j++) {
	...
	<h5>Public property consumables statistic</h5>
	<h5> since&nbsp;'; if (@$_GET['date1']=="") { echo date("Y/m/d"); } else { echo @$_GET['date1']; } ?> ~ <?php if (@$_GET['date2']=="") { echo date("Y/m/d"); } else { echo @$_GET['date2']; } echo 'Until </h5>
	...
	<table border="0" bgcolor="#ffffff" cellpadding="0" cellspacing="0" class="noborder2">
	  <tr>
		...
		<td align="center" width="7%" class="bLineB0">名稱name</td>
		<td align="center" width="5%" class="bLineB0">Unit</td>';
		foreach ($temp[$i0] as $k=>$v) {
			echo '<td align="center" width="3%" valign="top" class="bLineB0 '.($k==count($temp[$i0])-1?' bLineB01':'').'">'.getArkAreaName($v).'</td>';
		}
		echo '
	  </tr>';
		for ($patientstart = ($j-1)*$patientno; $patientstart < (($j-1)*$patientno)+$patientno; $patientstart++) {
		$v1 = $arrPatient[$patientstart];
			if($v1==0 && STK_NAME_s($v1)==""){
				echo "";
			}else{
				echo '
			  <tr>
				<td class="bLineB1">'.$v1.'</td>
				<td class="bLineB1">'.STK_NAME_s($v1).'</td>
				<td class="bLineB1">'.getUNIT($v1).'</td>';
				foreach ($temp[$i0] as $k=>$v) {
					echo '<td align="center" class="bLineB1 '.($k==count($temp[$i0])-1?' bLineB11':'').'">'.getSTK_NUM($_GET['date1'], $_GET['date2'], $v1, $v, 2, 1).'</td>';
				}
				echo '
			  </tr>';
			}  
