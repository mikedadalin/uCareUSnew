...
<table border="0" bgcolor="#ffffff" cellpadding="0" cellspacing="0">
  <tr>
	...
	$str .= "WHERE a.`type`='".$type."' and a.`STK_Date`>='".mysql_escape_string($_GET['date1'])."' and a.`STK_Date`<='".mysql_escape_string($_GET['date2'])."' AND a.IsStatus = 'M'";
	...
	if($db->num_rows() > 0){
	  ...
	  for ($i0=0;$i0<$db->num_rows();$i0++) {			
		...
		$str0 .= "WHERE a.`type`='".$type."' and a.`STK_Date`>='".mysql_escape_string($_GET['date1'])."' and a.`STK_Date`<='".mysql_escape_string($_GET['date2'])."' AND a.IsStatus = 'M'";
		  ...
		  if($db0->num_rows() > 0){
			  ...
			  for ($i=0;$i<$db0->num_rows();$i++) {			
				  ...
				  <table class="noborder2">
					<tr>
					  <td width="280" bgcolor="#FFFFFF">Bed number:<span id="log4"><?php if ($fid=="") { echo getBedID(getPID($r0['firmID'])); } else { echo getBedID(getPID($fid)); } ?></span></td>
					  <td bgcolor="#FFFFFF" align="right" width="320">  Since &nbsp;<?php if (@$_GET['date1']=="") { echo date("Y/m/d"); } else { echo @$_GET['date1']; } ?> ~ <?php if (@$_GET['date2']=="") { echo date("Y/m/d"); } else { echo @$_GET['date2']; } ?> Ends</td>
					</tr>
					<tr>
					  <td bgcolor="#FFFFFF">Resident: <?php if ($fid=="") { echo $r0['firmID']; } else { echo $fid; } ?>&nbsp;
<span id="log0"><?php if ($fid=="") { echo getPatientName(getPID($r0['firmID'])); } else { echo getPatientName(getPID($fid)); } ?></span>					  </td>
				  ...
				  <table width="100%" class="noborder">
					<tr class="bLineT bLineB">
					  ...
					  <td width="80" align="center">Delivery note ID#</td>
					  ...
					  <td width="55" align="center">Delivery $ amount</td>		
					</tr>'."\n";
				  }
				  $disQTY = $r0['QTY'];
				  $disPrice = $r0['Price'];
				  ...
				  if ($r0['type'].$r0['ordDate'].$r0['firmStockID']!=$tmp_no) {
					  ...
					  if (substr($r0['STK_NO'],0,1)=='3') { $TOT2 += $r0['STK_TOT']; } else { $TOT += $r0['STK_TOT']; }
				  }
				
				  echo '
				  <tr>
					<td align="center" width="80">'.$r0['STK_Date'].'</td>
					<td align="center" width="80">'.$count2.'&nbsp;'.$r0['type'].$r0['ordDate'].$r0['firmStockID'].'</td>
				  ...
				  if(($i==($db0->num_rows()-1))){	
					...
					<tr class="bLineB">
					<td colspan="8" align="right" width="615" >Subtotal:'.$db0->num_rows().' data ,出貨淨額： $ '.$NET.'，出貨稅額： $ '.$TAX.'，出貨總額： $ '.$TOT.'，代收款：$'.$TOT2.'&nbsp;</td>
					</tr>
					</table>';
				  }
				  if(($i+1)%15==0){$count1++;}
