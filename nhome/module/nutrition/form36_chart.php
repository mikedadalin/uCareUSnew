...
<title>U-ARK Nursing Information System</title>
...
<?php
if (@$_GET['date']=='') {
	$sql = "SELECT * FROM `socialform36` WHERE `HospNo`='".$HospNo."' ORDER BY `date` ASC";
} else {
	$sql = "SELECT * FROM `socialform36` WHERE `HospNo`='".$HospNo."' AND `date`<='".mysql_escape_string($_GET['date'])."' ORDER BY `date` ASC";
}
$db2a = new DB;
$db2a->query($sql);
$arrMMSE = array();
...
$(function () {
	$.plot($("#mmsechart"), [
		{ label: "MNA",  data: <?php echo $mmseTotal; ?> }
	],
	{
		lines: { show: true },
		points: { show: true },
