...
$arrNoSelDate = array("23", "24", "31", "34");
$arrNoQueryCol = array("23", "24", "31", "34", "8", "8a", "8b", "8c");
?>
...
<table align="center" style="width:100%; font-size:10pt; margin: 0px 0px;">
  <tr id="backtr" style="border:none; height:28px;">
    <?php 
	if (@$_GET['id']!=NULL) {
		echo '<td class="backbtnn" align="center" width="40" id="backbtn"  style="border:none;" rowspan="3"><a href="index.php?mod=nutrition&func=formview&pid='.mysql_escape_string($_GET['pid']).'">'.$word_Back.'</a></td>'; 
	}else{
		echo '<td class="backbtnn" align="center" width="40" id="backbtn"  style="border:none;" rowspan="3"><a href="index.php?func=nutritionlist" style="font-size:14px;">Resident List</a></td>';
	}
	?>
...
<table border="0" style="text-align:center; width:100%;" onclick="closeResidentCol();">
  <tr>
	...
	if (@$_GET['id']!=NULL) {
		if (!in_array(@$_GET['id'],$arrNoSelDate)) {
	        $formID = mysql_escape_string($_GET['id']);
	        if (strlen((int)$formID)==1) { $formID = '0'.$formID; }
			$tablename = 'socialform'.$formID;
			$db = new DB;
			$db->query("SELECT * FROM `".$tablename."` WHERE `HospNo`='".$HospNo."' ORDER BY `date` DESC");
			echo '<div id="tabs" style="padding-top:12px; width:100%;">'."\n";
			...
			foreach ($arrDate as $k=>$v) {
                ...
                <?php
				echo '<div id="tabs-'.$k.'" class="nurseform-table" style="padding:1px; font-size:11pt;"><center>';
				$tabsID=1;
				@$_GET['date']=$v;
