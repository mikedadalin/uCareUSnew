...
<?php
if (@$_GET['date']=='') {
	$sql = "SELECT * FROM `socialform13` WHERE `HospNo`='".mysql_escape_string($HospNo)."' ORDER BY `date` ASC";
} else {
	$sql = "SELECT * FROM `socialform13` WHERE `HospNo`='".mysql_escape_string($HospNo)."' AND `date`<='".mysql_escape_string($_GET['date'])."' ORDER BY `date` ASC";
}
$db2a = new DB;
$db2a->query($sql);
$arrQ1 = array();
...
for ($i=0;$i<$db2a->num_rows();$i++) {
	...
	for($Qi=1;$Qi<=6;$Qi++){
		for($Qj=1;$Qj<=5;$Qj++){
			//${'Q'.$Qi} += $r2a['Q'.$Qi.'_'.$Qj];
			if ($r2a['Q'.$Qi.'_'.$Qj]==1) { ${'Q'.$Qi} = $Qj; }
		}		
		${'arrQ'.$Qi}[number_format($second1970ms, 0, '.', '')] = ${'Q'.$Qi};
	}
}
for($Qi=1;$Qi<=6;$Qi++){
	ksort(${'arrQ'.$Qi});
	foreach (${'arrQ'.$Qi} as $k1=>$v1) { ${'txtQ'.$Qi} .= "[".$k1.",".$v1."],"; }
	${'txtQ'.$Qi}=substr(${'txtQ'.$Qi},0,strlen(${'txtQ'.$Qi})-1);
	${'txtQ'.$Qi} = '['.${'txtQ'.$Qi}.']';
}
?>
...
$(function () {
	$.plot($("#mmsechart"), [
		{ label: "入住意願及接受度",  data: <?php echo $txtQ1; ?>, bars: {order:1} },
		{ label: "家屬對機構的配合度",  data: <?php echo $txtQ2; ?>, bars: {order:2} },
		{ label: "Emotions",  data: <?php echo $txtQ3; ?>, bars: {order:3} },
		{ label: "Behavior",  data: <?php echo $txtQ4; ?>, bars: {order:4} },
		{ label: "社會化",  data: <?php echo $txtQ5; ?>, bars: {order:5} },
		{ label: "家庭支持力",  data: <?php echo $txtQ6; ?>, bars: {order:6} }
	],
	{
		bars: { show: true, barWidth : 24*60*60*1000 },
		xaxis: {
			mode: 'time', timeformat: "%y/%m", tickSize: [1, 'month']
		},
