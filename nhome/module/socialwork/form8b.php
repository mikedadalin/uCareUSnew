<div class="moduleNoTab">
	...
	if (isset($_POST['submit'])) {
		...
		$db0b = new DB;
		$db0b->query("UPDATE `socialform08` SET `actID`='".mysql_escape_string($_POST['actID'])."', `date`='".$date."', `HospNo`='".$hospnotxt."', `cUser`='".$_SESSION['ncareID_lwj']."' WHERE `actNo`='".mysql_escape_string($_GET['actNo'])."';");
		
		$db1b = new DB;
		$db1b->query("UPDATE `socialform08a` SET `Q1` = '".mysql_escape_string($_POST['Q1'])."', `Q2_1` = '".mysql_escape_string($_POST['Q2_1'])."', `Q2_5` = '".mysql_escape_string($_POST['Q2_5'])."', `Q2_2` = '".mysql_escape_string($_POST['Q2_2'])."', `Q2_6` = '".mysql_escape_string($_POST['Q2_6'])."', `Q2_3` = '".mysql_escape_string($_POST['Q2_3'])."', `Q2_7` = '".mysql_escape_string($_POST['Q2_7'])."', `Q2_4` = '".mysql_escape_string($_POST['Q2_4'])."', `Q2_8` = '".mysql_escape_string($_POST['Q2_8'])."', `Q3_1` = '".mysql_escape_string($_POST['Q3_1'])."', `Q4_1` = '".mysql_escape_string($_POST['Q4_1'])."', `Q4_2` = '".mysql_escape_string($_POST['Q4_2'])."', `Q4_3` = '".mysql_escape_string($_POST['Q4_3'])."', `Q4_4` = '".mysql_escape_string($_POST['Q4_4'])."', `Q4_5` = '".mysql_escape_string($_POST['Q4_5'])."', `Q5_1` = '".mysql_escape_string($_POST['Q5_1'])."', `Q5_2` = '".mysql_escape_string($_POST['Q5_2'])."', `Q5a` = '".mysql_escape_string($_POST['Q5a'])."', `Q6_1` = '".mysql_escape_string($_POST['Q6_1'])."', `Q6_2` = '".mysql_escape_string($_POST['Q6_2'])."', `Q6_3` = '".mysql_escape_string($_POST['Q6_3'])."', `Q6_4` = '".mysql_escape_string($_POST['Q6_4'])."', `Q7` = '".mysql_escape_string($_POST['Q7'])."', `Q8` = '".mysql_escape_string($_POST['Q8'])."', `Q8a` = '".mysql_escape_string($_POST['Q8a'])."', `Q9` = '".mysql_escape_string($_POST['Q9'])."', `Q10` = '".mysql_escape_string($_POST['Q10'])."', `Q11` = '".mysql_escape_string($_POST['Q11'])."' , `Q12` = '".mysql_escape_string($_POST['Q12'])."' WHERE `actNo`='".mysql_escape_string($_GET['actNo'])."'");
...
<form id="socialform08" action="index.php?mod=socialwork&func=formview&id=8b&actNo=<?php echo @$_GET['actNo']; ?>" method="post">
	<table width="100%">
		<tr>
			<td width="160" class="title">Select activity category</td>
			<td>
				<?php //echo $cateName . ' - ' . $actName; ?>
				<select id="cateName" onchange="changeAct(this.selectedIndex);">
					<option>--Activity category--</option>
					<?php
					foreach ($arrCateName as $k1=>$v1) {
						echo '<option value="'.$k1.'" '.($cateName==$v1?"selected":"").'>'.$v1.'</option>';
					}
					?>
				</select><br>
				<select name="actID" id="actID" class="validate[required]" readonly>
					...
					<?php
					$db2c = new DB;
					$db2c->query("SELECT * FROM `socialform08_act` WHERE `cateName`='".$cateName."' ORDER BY `actID`");
					for ($i2c=0;$i2c<$db2c->num_rows();$i2c++) {
						$r2c = $db2c->fetch_assoc();
						echo '<option value="'.$r2c['actID'].'" '.($actName==$r2c['actName']?"selected":"").'>'.$r2c['actName'].'</option>'."\n";
					}
					?>
				</select>
			</td>
			<td width="150" class="title">Date</td>
			<td width="260"><script>$(function() { $( "#date").datetimepicker({format:'Y/m/d', timepicker: false, mask: true}); });</script><input type="text" name="date" id="date" size="10" value="<?php echo $date==NULL?date("Y/m/d"):$date; ?>" class="validate[required,custom[date]]" />&nbsp;Time: <input type="text" id="Q12" name="Q12" value="<?php echo $Q12;?>" size="8"></td>
		</tr>
		<tr>
				...
				foreach ($arrHospNo as $k1=>$v1) {
					if (count($arrHospNo[$k1])>0) {
						...
						foreach ($arrHospNo[$k1] as $k2=>$v2) {
							...
							echo '<div style="width:220px; display:inline-block;"><input type="checkbox" name="HospNo[]" value="'.getHospNo($v2).'" class="validate[minCheckbox[1]] checkbox"'.(in_array(getHospNo($v2),$arrSelectedHospNo)?" checked":"").'  onclick="calcNo(\'HospNo[]\')"> '.($cateName == "輔療性團體活動"?'<a href="index.php?mod=socialwork&func=formview&pid='.$v2.'&id=11c&cate='.$_GET['actNo'].'" target="_blank">':"").getHospNo($v2).' '.getPatientName($v2).($cateName == "輔療性團體活動"?'</a>':'').' </div>';
							$count1++;
						}
					}
				}
		...
		<tr>
			...
			<td><input type="text" id="Q7" name="Q7" size="3" value="<?php echo $Q7; ?>" readonly></td>
		</tr>
		<tr colspan="3" style="text-align:left;">
			...
			<td colspan="3">
				...
				<input type="text" id="Q8a" name="Q8a" size="30" value="<?php echo $Q8a;?>"></td>
			</tr>
			<tr colspan="3" style="text-align:left;">
				<td class="title">
					<?php
					...
					?></td>
					<td colspan="3"><input type="text" id="Q9" name="Q9" size="60" value="<?php echo $Q9; ?>"></td>
				</tr>
				<tr colspan="3" style="text-align:left;">
				...
				<tr colspan="3" style="text-align:left;">
					...
					<td colspan="3"><input type="text" id="Q11" name="Q11" size="60" value="<?php echo $Q11; ?>"></td>
				</tr>
				<tr>
