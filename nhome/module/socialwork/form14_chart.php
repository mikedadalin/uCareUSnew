...
<?php
if (@$_GET['date']=='') {
	$sql = "SELECT * FROM `socialform14` WHERE `HospNo`='".mysql_escape_string($HospNo)."' ORDER BY `date` ASC";
} else {
	$sql = "SELECT * FROM `socialform14` WHERE `HospNo`='".mysql_escape_string($HospNo)."' AND `date`<='".mysql_escape_string($_GET['date'])."' ORDER BY `date` ASC";
}
$db2a = new DB;
$db2a->query($sql);
$arrQ1 = array();
...
$(function () {
	$.plot($("#mmsechart"), [
		{ label: "Physiological function",  data: <?php echo $txtQ1; ?> },
		{ label: "Mental Status",  data: <?php echo $txtQ2; ?> },
		{ label: "Behavior status",  data: <?php echo $txtQ3; ?> },
		{ label: "Social interaction",  data: <?php echo $txtQ4; ?> }
	],
	{
		lines: { show: true },
		points: { show: true },
