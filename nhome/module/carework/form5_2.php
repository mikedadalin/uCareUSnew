<?php
if (@$_GET['date']=='') {
	$sql = "SELECT * FROM `careform05_2` WHERE `HospNo`='".$HospNo."' AND `date`=''";
} else {
	$sql = "SELECT * FROM `careform05_2` WHERE `HospNo`='".$HospNo."' AND `date`='".mysql_escape_string($_GET['date'])."'";
}
$db1 = new DB;
$db1->query($sql);
$r1 = $db1->fetch_assoc();
...
<center>
  <div style="margin-top:20px">
    <input type="hidden" name="formID" id="formID" value="careform05_2" /><input type="hidden" name="HospNo" id="HospNo" value="<?php echo $HospNo; ?>" /><input type="button" id="back" value="Back to list" /><button type="submit" id="submit_<?php echo $_GET['id']?>" class="SubmitCloud" onclick="window.onbeforeunload=null;return true;"><i class="fa fa-cloud-upload fa-2x"></i>Submit</button><button type="button" id="VNFormOpen" class="ButtonVN" onclick="openVerificationForm2('<?php echo $_GET['id'] ;?>');"><div><i class="fa fa-cloud-upload fa-2x"></i> <i class="fa fa-key"></i></div>Password Required</button>
  </div>
</center>
</div>
</form>
