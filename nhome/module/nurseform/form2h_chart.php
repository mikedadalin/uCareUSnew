...
<?php
if (@$_GET['date']=='') {
	$sql = "SELECT * FROM `nurseform02h` WHERE `HospNo`='".mysql_escape_string($HospNo)."' ORDER BY `date` ASC";
} else {
	$sql = "SELECT * FROM `nurseform02h` WHERE `HospNo`='".mysql_escape_string($HospNo)."' AND `date`<='".mysql_escape_string($_GET['date'])."' ORDER BY `date` ASC";
}
$db2a = new DB;
$db2a->query($sql);
$arrMMSE = array();
for ($i=0;$i<$db2a->num_rows();$i++) {
	...
	$arrMMSE[number_format($second1970ms, 0, '.', '')] = $r2a['Q32'];
}
ksort($arrMMSE);
...
$mmseTotal = '['.$mmseTotal.']';
?>
<div id="mmsechart" style="width:800px;height:150px; margin-left:20px;"></div>
<script type="text/javascript">
$(function () {
	$.plot($("#mmsechart"), [
		{ label: "MMSE",  data: <?php echo $mmseTotal; ?> }
	],
	{
		lines: { show: true },
		points: { show: true },
