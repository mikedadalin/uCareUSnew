<?php
$db = new DB;
$db->query("SELECT * FROM `nurseform17` WHERE `HospNo`='".$HospNo."' AND `date`='".mysql_real_escape_string(@$_GET['date'])."' AND `drugID`='".mysql_real_escape_string(@$_GET['dID'])."'");
if ($db->num_rows()>0) {
	$r = $db->fetch_assoc();
	foreach ($r as $k=>$v) {
...
?>
<form action="index.php?func=medwork&pid=<?php echo @$_GET['pid']; ?>&action=new" method="post" onSubmit="return checkForm();">
<h3>New medication</h3>
    <table>
      <tr>
        <td class="title" width="120">Medication</td>
        <td colspan="3"><?php echo $Qmedicine ?><input type="hidden" name="drugID" id="drugID" value="<?php echo $drugID; ?>" ></td>
      </tr>
      <tr>
	 ...
	 <td colspan="2"><input type="text" size="6" name="Qway" id="Qway" value="<?php echo $Qway; ?>" /></td>
      ...
      <tr>
        ...
        <td><select onchange="selectmedtime(this.value);" id="freqselect">
          ...
          <?php
		  $db1 = new DB;
		  $db1->query("SELECT * FROM `medfreq` ORDER BY `avaliable`, `code` ASC");
      ...
      <tr>
        ...
        <td colspan="3"><input type="text" name="Qusage" id="Qusage" size="3" value="<?php echo $Qusage; ?>" /></td>
      </tr>
      <tr>
        <td class="title">Medication time</td>
      ...
      <tr>
        <td class="title">Medication day</td>
        <td colspan="3"><?php echo draw_option("Qmedday","Mon;Tue;Wed;Thu;Fri;Sat;Sun;Every day;Every 2 day","m","multi",'',false,7); ?></td>
      </tr>
      <tr>
        <td class="title">Start date</td>
        <td><script> $(function() { $( "#Qstartdate").datetimepicker({format:'Y/m/d', timepicker: false, mask: true}); }); </script>
          <input type="text" name="Qstartdate" id="Qstartdate" value="<?php if ($Qstartdate != NULL) { echo $Qstartdate; } else { echo date('Y/m/d'); } ?>" size="12" /></td>
        <td class="title">End date</td>
        <td><script> $(function() { $( "#Qenddate").datetimepicker({format:'Y/m/d', timepicker: false, mask: true}); }); </script>
          <input type="text" name="Qenddate" id="Qenddate" value="<?php if ($Qenddate != NULL) { echo $Qenddate; } ?>" size="12" /></td>
      </tr>
    </table>
<table width="100%">
  <tr>
