...
<?php
if (@$_GET['date']=='') {
	$sql = "SELECT * FROM `nurseform02d` WHERE `HospNo`='".mysql_escape_string($HospNo)."' ORDER BY `date` ASC";
} else {
	$sql = "SELECT * FROM `nurseform02d` WHERE `HospNo`='".mysql_escape_string($HospNo)."' AND `date`<='".mysql_escape_string($_GET['date'])."' ORDER BY `date` ASC";
}
$db2a = new DB;
$db2a->query($sql);
$arrMMSE = array();
...
$(function () {
	$.plot($("#mmsechart"), [
		{ label: "SPMSQ",  data: <?php echo $mmseTotal; ?> }
	],
	{
		lines: { show: true },
		points: { show: true },
