...
if($db->num_rows()>0){
	$r = $db->fetch_assoc();
	$arr_PermissionGroup = explode(";",$r['PermissionGroup']);
	if(!in_array($_SESSION['ncareGroup_lwj'],$arr_PermissionGroup) || $r['PermissionLevel']=="" || ($r['PermissionLevel']==2 && $_SESSION['ncareLevel_lwj']!=5)){
...
$arrNoSelDate = array("");
$arrNoHeader = array(""); //不列印住民資訊抬頭
$arrNoMagic = array("");
$arrNoResidentList = array("");
?>

<div onclick="closecol();">
<div ondblclick="closeResidentCol();">
<?php
if (!in_array($_GET['id'],$arrNoHeader) && substr($url[3],0,5)!="print") {
?>
...
<table align="center" style=" width:100%; font-size:10pt; margin: 0px 0px;">
  <tr id="backtr" style="border:none; height:28px;">
	...
	if (@$_GET['id']!=NULL) {
		if ($_SESSION['ncareGroup_lwj']!=4) {
			echo '<td class="backbtnn" align="center" width="40" id="backbtn"  style="border:none;" rowspan="3"><a href="index.php?mod=nurseform&func=formview&pid='.mysql_escape_string($_GET['pid']).'">Back To List</a></td>';
		} else {
    ...
    <td class="title" width="60" style="border:none;">Name</td>
    ...
    <td width="100" class="title" style="border:none;">Admission Date</td>
  ...
  <tr style="border:none; height:28px;" >  <!-- 診斷 + Jump to + 時間 + Print -->
	...
	if ($db_remind->num_rows()>0) {
		echo '
		<td class="title" style="background-color:#b79810;">Diagnosis</td>
		<td style="padding-left: 10px;" colspan="9">'.$diagMsg.'</td>
		';
	}else{
		echo '<td class="title" style="background-color:#b79810; border-bottom-left-radius:10px;">Diagnosis</td>';
  ...
  if ($db_remind->num_rows()>0) {
	  if(substr($url[3],0,5)!="print"){
		  if(substr($_SESSION['ncareID_lwj'],0,8)!="resident"){
			  echo '
		      <tr class="printcol">
	            <td class="title" width="70" style="border:none; background-color:#e87217; border-bottom-left-radius:10px;">Reminders</td>
  ...
  <tr>
	...
	if (@$_GET['id']!=NULL) {
		if (!in_array(@$_GET['id'],$arrNoSelDate)) {
			...
			echo '<center><div id="tabs" style="padding-top:12px;">'."\n";
			...
			for ($i=1;$i<=$db->num_rows();$i++) {
				...
				echo '<li><a href="#tabs-'.$i.'">'.formatdate($r['date']).$no.'<br>'.(checkusername($r['Qfiller'])==""?"&nbsp;":checkusername($r['Qfiller'])).'</a>';
				if(substr($_SESSION['ncareID_lwj'],0,8)!="resident"){  /*== 居民判斷 ==*/
