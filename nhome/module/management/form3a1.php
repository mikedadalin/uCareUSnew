	...
	$(function() {
		$( "#dialog-form1" ).dialog({
			...
			buttons: {
				"Add record": function() {
					$.ajax({
						url: "class/saveTargetPatNo2.php",
						type: "POST",
	...
	$( "#dialog-form2" ).dialog({
		...
		buttons: {
			"Edit record": function() {
				$.ajax({
					url: "class/editTargetPatNo2.php",
					type: "POST",
...
<table width="100%" id="pNoTable">
	...
	if ($_GET['date1']==NULL && $_GET['date2']==NULL) {
		$sql = "SELECT * FROM `dailypatientno1` ORDER BY `date` DESC";
	} else {
		$sql = "SELECT * FROM `dailypatientno1` WHERE `date` BETWEEN '".mysql_escape_string($_GET['date1'])."' AND '".mysql_escape_string($_GET['date2'])."' ORDER BY `date` DESC";
	}
	$db0 = new DB;
	$db0->query($sql);
	for ($i0=0;$i0<$db0->num_rows();$i0++) {
		...
		<td align="center">'.$r0['date'].'</td>
		<td align="center">'.$r0['newpat'].'</td>
		<td align="center">'.$r0['outpat'].'</td>
		<td align="center">'.(str_replace('-','',$r0['date'])<'20150401' && $r0['foleypat']==0?"---":$r0['foleypat']).'</td>
		<td align="center">'.(str_replace('-','',$r0['date'])<'20150401' && $r0['nofoleypat']==0?"---":$r0['nofoleypat']).'</td>
		<td align="center">'.(str_replace('-','',$r0['date'])<'20150401' && $r0['hosppat']==0?"---":$r0['hosppat']).'</td>
		<td align="center">'.(str_replace('-','',$r0['date'])<'20150401' && $r0['backpat']==0?"---":$r0['backpat']).'</td>
		<td align="center">'.(str_replace('-','',$r0['date'])<'20150401' && $r0['deadpat']==0?"---":$r0['deadpat']).'</td>
		<td align="center">'.$r0['no'].'</td>
		<td align="center">'.checkusername($r0['Qfiller']).'</td>
		</tr>
		'."\n";
	}
	?>
	...
	<form id="form1">
		<fieldset>
			...
			<!--<tr>
				...
				<tr>
					<td class="title">Discharge (checked-out)</td>
