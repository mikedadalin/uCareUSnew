...
if (@$_GET['date']=="--Select month--") {
	...
	$qdate_vital = substr(@$_GET['date'],0,4).substr(@$_GET['date'],4,2);
}
$db = new DB;
$db->query("SELECT * FROM `vitalsign` WHERE `PatientID`='".mysql_escape_string($_GET['pid'])."' AND `date` LIKE '".$qdate_vital."%' ORDER BY `date` ASC,`time` ASC");
for ($i=1;$i<=31;$i++) {
