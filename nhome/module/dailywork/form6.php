...
<div class="moduleNoTab">
	...
	<form action="index.php?func=database&action=save">
		<input type="button" id="newrecord" value="New meeting minute" />
	</form>
	...
	<div style="width:100%;">
		<table style="width:100%" id="recordlist">
			...
			<tbody>
				<?php
				$db2 = new DB;
				$db2->query("SELECT * FROM `nurseform06a` WHERE `Qcate`='2' ORDER BY `date`, `time` DESC");
				for ($i=0;$i<$db2->num_rows();$i++) {
					$r2 = $db2->fetch_assoc();
					echo '
					<tr>
					<td><center><a href="index.php?mod=dailywork&func=formview&id=6_1&date='.$r2['date'].'&time='.$r2['time'].'"><img src="Images/edit_patient.png" width="20" border="0"></a></center></td>
