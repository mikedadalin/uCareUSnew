...
if (isset($_POST['saveproduct'])) {
	...
	foreach ($_POST as $k=>$v) {
		...
		if (count($arrVarName)==2) {
			...
			//echo "UPDATE `arkstock` SET `".$arrVarName[0].'_'.$arrVarName[1]."`='".$vtowrite."' WHERE `STK_NO`='".mysql_escape_string($_POST['STK_NO'])."'<br>";
			  ...
			  //申請物品管理也一起設定 modify by Rowena 140910
			  if($arrVarName[0].'_'.$arrVarName[1] == "STOP_ID"){			
				$db1 = new DB;
				$db1->query("UPDATE `arkstockforapply` SET `STOP_ID`='".$vtowrite1."' WHERE `STK_NO`='".mysql_escape_string($_POST['STK_NO'])."'");				
			  }
			  //申請物品管理也一起設定
			}
		}
	...
	//print_r($_POST);
	echo "<script>alert('編輯商品成功！');</script>";
}

$STK_NO = mysql_escape_string($_GET['STKNO']);
$db = new DB;
$db->query("SELECT * FROM `arkstock` WHERE `STK_NO`='".$STK_NO."'");
$r = $db->fetch_assoc();
?>
<form method="post" action="index.php?mod=consump&func=formview&id=11_1&STKNO=<?php echo $_GET['STKNO']; ?>">
<table>
  <tr>
  ...
  <tr>
    ...
    <td align="left"><?php echo $r['STK_NO']; ?><input type="hidden" name="STK_NO" id="STK_NO" value="<?php echo $r['STK_NO']; ?>"></td>
    <td class="title">Name</td>
    <td colspan="3" align="left"><input type="text" name="STK_NAME" id="STK_NAME" value="<?php echo $r['STK_NAME']; ?>" size="60"></td>
    
  </tr>
  <tr>
    <td class="title">Spec.</td>
    <td align="left"><input type="text" name="STK_SPK" id="STK_SPK" size="15" value="<?php echo $r['STK_SPK']; ?>"></td>
    <td class="title">Model</td>
    <td align="left"><input type="text" name="STK_MODEL" id="STK_MODEL" size="20" value="<?php echo $r['STK_MODEL']; ?>"></td>
  ...
  <tr>
    ...
    <td align="left">$ <input type="text" name="IN_PRC" id="IN_PRC" size="15" value="<?php echo $r['IN_PRC']; ?>"></td>
  ...
  <tr>
    ...
    <td align="left"><?php echo $r['OUT_DATE']; ?></td>
  ...
  <tr class="printcol">
    <td colspan="6" align="center"><input type="button" value="Back to list" onclick="window.location.href='index.php?mod=consump&func=formview&id=11'"> <input type="submit" name="saveproduct" id="saveproduct" value="Save changes"></td>
  </tr>
</table>
</form>