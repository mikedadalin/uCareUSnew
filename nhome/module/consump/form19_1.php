...
if (isset($_POST['submit'])) {
	...
	if ($_POST['firmstockID']!=NULL) {
		...
		if ((int) @$_POST['oldCount1'] <> ""){
			...
			for ($i=1; $i<=$fCount ;$i++) {
				...
				if($tmp_STK_NO <> 0 && $_POST['oldSTOCK_INFO_NAME'.$i] <> '')
				{
				  ...
				  //新增明細
				  $db2 = new DB;
				  $strQry ="INSERT INTO `firmstockexpire_op` (`firmStockID`,`STK_DATE`,`infoOrd`,`STK_NO`,`QTY`,`cdate`,`eDate`,`StockID`,`type`,`userID`";
				  $strQry .=") VALUES (";
				  ...
				  $strQry .=" '".mysql_escape_string($_POST['oldcdate'.$i])."',";
		...
		echo "<script>alert('Add successfully!');window.location.href='index.php?mod=consump&func=formview&id=19'</script>";
	}else{
		echo "<script>alert('Data error!');window.location.href='index.php?mod=consump&func=formview&id=19'</script>";
	}
}
...
include("class/blockTableExpOp.php");
 ?>
</div>
<br />&nbsp;
<center>
...
$(function() {
	...
	$('#cmdBack').click(function(){
	  location.href='index.php?mod=consump&func=formview&id=19' ;
	});
