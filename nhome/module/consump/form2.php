...
<h3>Items Requisitions (Nursing department)</h3>
<?php
...
if (@$_GET['action']=="save") {
	...
	foreach ($_POST as $k=>$v) {
		if (substr($k,0,4)=="item") {
			...
			if ($v!=0) {
				$db = new DB;
				$db->query("INSERT INTO `consumpform02` VALUES ('".$itemID."', '".$date."', '".$v."', '".$_SESSION['ncareID_lwj']."')");
			}
		}
	}
}
...
?>
<form  method="post" onSubmit="return checkForm();" action="index.php?mod=consump&func=formview&id=2&action=save">
<table width="100%">
  <tr>
...
for ($i=1;$i<=$rowno;$i++) {
	...
	if (@$_GET['date']!=NULL) {
		$db1a = new DB;
		$db1a->query("SELECT `quantity` FROM `consumpform02` WHERE `itemID`='".$arrVar[$no1]."' AND `date`='".mysql_escape_string($_GET['date'])."'");
	...
	if (@$_GET['date']!=NULL) {
		$db2a = new DB;
		$db2a->query("SELECT `quantity` FROM `consumpform02` WHERE `itemID`='".$arrVar[$no2]."' AND `date`='".mysql_escape_string($_GET['date'])."'");
