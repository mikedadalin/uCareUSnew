...
if($_GET["firmStockID"]!=""){
	...
	$strQry .= " AND YEAR(  `STK_Date` ) ='".$tmpDate1."'";
...
if($_POST["act"]=="delete"){
	...
	if($db->num_rows()>0){			
		for ($ii=0;$ii<$db->num_rows();$ii++) {
			...
			$strQry1 .="`OUT_PRC`=OUT_PRC-".$rs['QTY'] * $rs['Price'].", ";
			$strQry1 .="`uDate`='".date("Y-m-d H:i:s")."', ";
			$strQry1 .="`userID`='".$_SESSION['ncareID_lwj']."' ";
...
$sql1="";
$sql1="SELECT * FROM ";
$sql1.="(SELECT t1.*,b.title tTitle, RIGHT( EXTRACT( YEAR_MONTH FROM  t1.`STK_Date` ) , 4 ) ordDate from `".$strModule."` t1 inner join `firm` b on t1.firmID=b.firmID where t1.IsStatus<>'D' and t1.type='".$type."' ".$strQry."  ORDER BY t1.`".$strModule."ID` DESC LIMIT 0, ".abs($totalRecord-($pn-1)*$ps)." ) AS T2";
...
<table width="100%" class="printcol" cellpadding="6" style="margin-bottom:20px;">
  ...
  <tr>
    <td width="120" class="title"><center><b>Return Bill #</b></center></td>
...
<input type="button" name="btnAdd" id="btnAdd" value=" Add Return Bill " />
</div>
...
<tr class="title">
  <td width="50">&nbsp;</td>
  <td width="100">Return bill #</td>
  <td width="100">Return date</td>
  <td width="200">Vendor's name</td>
  <td width="200">Return total value</td>
...
<tr>
  ...
  <td align="right"><?php echo $r['ReceiptNO']; ?>&nbsp;</td>
  <td>
  	<input type="button" value="Edit" onclick="window.location.href='index.php?mod=consump&func=formview&id=8_1a&firmstockID=<?php echo $firmStockID; ?>&STK_DATE=<?php echo $r['STK_Date']; ?>'">
</td>
</tr>
<?php }}else{?>
<tr>
...
<!--page Start-->
<table width="100%" style="border-collapse: collapse;" cellpadding="13" class="printcol">
  <tr>
    <td align="center" class="title page">
	<?php
		changePageManager($totalRecord,$totalPage,$pn,$ps,$pageString,"index.php?mod=consump&func=formview&id=8_2a");
    ?>
    </td>
  </tr>
</table>
...
$(function() {
  $('#btnSearch').click(function() {
	...
	$('form').attr('action', "index.php?mod=consump&func=formview&id=8_2a&firmStockID="+$('#firmStockID').val()+"&firmID="+$('#firmID').val()+"&sSTK_Date="+$('#sSTK_Date').val());
	$('form').submit();
  });

  $('#btnAdd').click(function() {
	var page = "module/consump/firm.php"  
