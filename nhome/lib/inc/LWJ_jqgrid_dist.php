 ...
 * @version 1.5.2 build 20140509-1253
...
class jqgrid
{
	...
	function jqgrid($db_conf = null)
	{
		...
		$grid["width"] = 1490;
		$grid["height"] = 550;
		$grid["rowList"] = array(5,10,15,20,25,30,'All');
	...
	private function to_sql($field, $oper, $val) 
	{
		...
		else if ($oper=='inline') return addslashes($val);
	...
	function render($grid_id)
	{
		...
		if (isset($_GET['jqgrid_page']))
		{
			...
			foreach ($rows as $row)	
			{
				...
				foreach($row as $k=>$r)
						/*== 解 START ==*/
						if($k=="Name1" || $k=="Name2" || $k=="Name3" || $k=="Name4" || $k=="ResidentCardNo" || $k=="PassportNo"){
	    					$rsa = new lwj('lwj/lwj');
	    					$puepart = explode(" ",$r);
