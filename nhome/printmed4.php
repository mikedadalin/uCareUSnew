...
td { word-break:normal; word-wrap:normal; }
</style>
</head>
...
for ($page=1;$page<=$pageno;$page++) {
	...
	$rowno = $pageno*$drugno;
?>
<table style="border-collapse:collapse; font-size:10pt;" width="1480">
  <tr id="backtr"  style="border:none;" height="20">
    <td nowrap style="border:none;"><?php echo $_SESSION['nOrgName_lwj'].' '.substr($qdate,0,4).' Year '.substr($qdate,4,2); ?> 月給藥紀錄單</td>
    <td nowrap class="title" width="60" style="border:none;">Bed #</td>
    <td nowrap style="border:none;"><?php echo $bed; ?></td>
    <td nowrap class="title" width="60" style="border:none;">Full name</td>
    <td nowrap style="border:none;"><?php echo $name; ?></td>
    <td nowrap class="title" width="60" style="border:none;">Care ID#</td>
    <td nowrap style="border:none;"><?php echo $HospNo; ?></td>
    <td nowrap class="title" width="60" style="border:none;">DOB</td>
    <td nowrap style="border:none;"><?php echo $birth.' ('.checkgender(mysql_escape_string($_GET['pid'])).', '.calcage(str_replace('/','',$birth)).')'; ?></td>
...
<table border="1" style="border-collapse:collapse; text-align:center;" width="1480">
  <tr class="title" height="30">
    ...
    <td width="50">起訖</td>
    <td width="146">Medication (Dose)</td>
    <td width="50">Time</td>
    <?php
  ...
  for ($i=0;$i<$stopno;$i++) {
	  ...
	  echo '
  <tr>
    <td rowspan="'.$rowspan.'">'.$r['Qsource'].'</td>
	<td rowspan="'.$rowspan.'">'.formatshortdate(str_replace("/","",$r['Qstartdate'])).'<br>'.formatshortdate(str_replace("/","",$r['Qenddate'])).'</td>
    <td rowspan="'.$rowspan.'">'.$r['Qmedicine'].' '.$r['Qdose'].$r['Qdoseq'].'<br>'.$r['Qusage'].', '.$r['Qway'].', '.$r['Qfreq'].($r['Qeffect']!=""?'<br>作用：'.$r['Qeffect']:'<br>作用：').'</td>
    <td height="32">'; if (count($time)>0 && $time[0]<=9) { echo $time[0].'am'; } else { echo '&nbsp;'; } echo '</td>'.drawmedcal($bgday,$r['Qmedday']).'
  </tr>'."\n";
    if (count($time)<=4) {
		$time2 = '&nbsp;';
		$time3 = '&nbsp;';
		...
		echo '
		<tr height="32"><td>'.$time2.'</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>
		<tr height="32"><td>'.$time3.'</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>
		<tr height="32"><td>'.$time4.'</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>
		';
	} elseif (count($time)==0) {
		echo '<tr height="32"><td>&nbsp;</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>'."\n";
		echo '<tr height="32"><td>&nbsp;</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>'."\n";
		echo '<tr height="32"><td>&nbsp;</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>'."\n";
	} else {
		for ($t1=0;$t1<count($time);$t1++) {
			echo '<tr height="32"><td>'.$time[$t1].'</td>'.drawmedcal($bgday,$r['Qmedday']).'</tr>'."\n";
		}
	}
  ...
  ?>
  <tr>
    <td colspan="3" rowspan="3" class="title"><font size="2">護理人員簽章</font></td>
    <td class="title" height="32"><font size="2">Day shift</font></td>
    <?php echo drawmedcal('',''); ?>
  </tr>
  <tr height="32">
    <td class="title"><font size="2">Night shift</font></td>
    <?php echo drawmedcal('',''); ?>
  </tr>
  <tr height="32">
    <td class="title"><font size="2">Graveyard shift</font></td>
    <?php echo drawmedcal('',''); ?>
  </tr>
  <tr height="20">
    <td colspan="4" class="title"><font size="2">不良反應</font></td>
    <?php echo drawmedcal('',''); ?>
  </tr>
</table>
<?php
	if ($pageno==$page) {
		echo '<p style="margin:0px;"><font size="2">Abbreviation: Not through mouth-NPO　Refuse - Ref　Outgoing - Out　睡眠 - Slp　Shortage of drug -A　不良反應 - O/X　Other-＊ (Noted in Nursing records)</font></p>';
	} else {
		echo '<p style="page-break-after: always; margin:0px;"><font size="2">Abbreviation: Not through mouth-NPO　Refuse - Ref　Outgoing - Out　睡眠 - Slp　Shortage of drug -A　不良反應 - O/X　Other-＊ (Noted in Nursing records)</font></p>';
	}
}
?>
</body>
