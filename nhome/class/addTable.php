...
function S_AMT(id) { //計算金額 Quantity*Unit price calculate = number* unit price
	var QTY = parseFloat($('#dQTY'+id).val());
	var PRC = parseFloat($('#IN_PRC'+id).val());
...
function S_AMT1() { //移除金額 Quantity*Unit price remove amount($) = number * unit price
...
function S_TOT(net,t) { //計算總額 淨額+稅額calculate total price= net amount+tax amount
...
$(function () {
  ...
  $('#addFile').click(function() {
	...
	if($("#firmID").val()==''){alert('請先選擇廠商please select vendor!'); $("#firmID").focus(); return;}
    count++;
	dcount++;
	//1st
	var data = ["－－－","chargable category","no direct charge category","proxy charge","kitchen ingredient.supply","other department proxy ordering","donation","property maintenance","property","donate supply"];
	var tmp=new Array()
	for(i=0;i<10;i++){
		tmp[0]="<option value='"+i+"'>"+data[i]+"</option>";
		tmp[1]=tmp[1]+tmp[0];
	}
    ...
    var myTableDom = [{
      tagName:'table', id:'block' + count, className:'a1', childNodes:[{
        tagName:'tbody', childNodes:[{
          ...
          tagName:'tr', childNodes:[{            
              tagName:'td', width:'80', childNodes:[{//product serial number	product serial number					  
			  tagName:'input', type:'text', align:'middle', size:'8', id:'STK_NO' + count, name:'STK_NO' + count, onblur:'blurselect('+count+')'}]},{
			  ...
			  tagName:'input', type:'text', size:'6', id:'IN_PRC' + count, name:'IN_PRC' + count, onblur:'S_AMT('+count+');'}]},{
			  ...
			  tagName:'input', type:'text', size:'5', id:'STOCK_INFO' + count, name:'STOCK_INFO' + count, onblur:'stockINFO('+count+');this.value = this.value.toUpperCase();'},{
			  ...
			  tagName:'input', type:'text', size:'8', id:'STOCK_INFO_NAME' + count,name:'STOCK_INFO_NAME' + count, readonly:'readonly'},]},{
