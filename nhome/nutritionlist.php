...
$r = $db->fetch_assoc();
$url = "index.php?mod=nutrition&func=formview&pid=".$r['patientID'];
echo "<script type='text/javascript'>";
echo "window.location.href='".$url."'";
echo "</script>";
}else{
$db3 = new DB;
$db3->query("SELECT `HospNo`, `Q12a`, `Q13a`, `Q14a` FROM `socialform32` WHERE (`Q8` IS NULL OR `Q8`='') AND (`Q9` IS NULL OR `Q9`='') AND (`Q10` IS NULL OR `Q10`='')");
$db4 = new DB;
$db4->query("SELECT * FROM `nurseform31` WHERE `Q3_3`='1'");
$count = 0;
?>
...
<div id="slider_scroll">
    ...
    <div id="slider_content">
      <div class="patlistbtn"><a href="index.php?mod=nutrition&func=formview&id=23" title="Body weight change"><i class="fa fa-line-chart fa-2x fa-fw"></i><br>Change of Weight</a></div>
      <div class="patlistbtn"><a href="index.php?mod=nutrition&func=formview&id=24" title="Body weight statistic"><i class="fa fa-tasks fa-2x fa-fw"></i><br>Stat. of Weight</a></div>
      <div class="patlistbtn"><a href="index.php?mod=nutrition&func=allpatient" title="Weight record"><i class="fa fa-pencil-square-o fa-2x fa-fw"></i><br>Weight Record</a></div>
      <div class="patlistbtn"><a href="index.php?mod=mealadmin&func=foodmenu" title="Meal menu"><i class="fa fa-cutlery fa-2x fa-fw"></i><br>Meal menu</a></div>
      <div class="patlistbtn">
        <a class="patlistlink" title="Nursing notify"><i class="fa fa-files-o fa-2x fa-fw"></i><br>Nursing notify</a>
        <?php
        if ($db3->num_rows()>0 || $db4->num_rows()>0) {
          ...
          for ($i3=0;$i3<$db3->num_rows();$i3++) {
            ...
            if ($i3>0) { echo '<br>'; }
            echo '<a href="index.php?mod=nutrition&func=formview&pid='.$pid.'&id=32" title="Nutritional notify">Nutritional notify<br>'.getBedID($pid).'<br>'.getPatientName($pid).'</a><br>-----------';
          }
          echo '</div>';
        }
        ?>
          ...
          for ($j=0;$j<$db1->num_rows();$j++) {
              ...
              if (count($arrAreaBed)==0 || in_array($r1['bed'],$arrAreaBed)) {
                  ...
                  echo '>
                    <td class="link1">
                      <center>
                      <a href="index.php?mod=nutrition&func=formview&pid='.$r['patientID'].'&query='.@$_GET['query'].'" title="Edit" onmouseover="showPatientPic('.$r['patientID'].');" onmouseout="hiddenPatientPic('.$r['patientID'].');"><i class="fa fa-magic fa-lg"></i></a>
                      </center>
                    </td>
                    <td>'.$r2b['areaName'].'</td>
                    <td>'.$r1['bed'].'</td>
	...
	$('table[id^=patlist]').dataTable({
		...
		"drawCallback": function ( settings ) {
            ...
            api.column(1, {page:'current'} ).data().each( function ( group, i ) {
                if ( last !== group ) {
                    $(rows).eq( i ).before(
                        '<tr class="group"><td colspan="8" style="line-height:24px; background:#efefef;"><i class="fa fa-compass fa-lg"></i> '+group+'</td></tr>'
                    );
