...
<table width="800" class="drawformborder" cellpadding="0" cellspacing="0" style="font-size:14pt;">
  ...
  <tr>
	...
	if ($Startdate1!=NULL && $Enddate1!=NULL) {
		...
		echo 'since '.($startdate[0]-1911).' 年year '.$startdate[1].' 月month '.$startdate[2].' date日至untill '.($enddate[0]-1911).' 年year '.$enddate[1].' 月month '.$enddate[2].' 日止date<br>';
	}
	if ($Startdate2!=NULL && $Enddate2!=NULL) {
		$startdate = explode("/",$Startdate2);
		$enddate = explode("/",$Enddate2);
		echo 'since '.($startdate[0]-1911).' 年year '.$startdate[1].' 月month '.$startdate[2].' date日至until '.($enddate[0]-1911).' 年year '.$enddate[1].' 月month '.$enddate[2].' 日止date<br>';
	}
	if ($Startdate3!=NULL && $Enddate3!=NULL) {
		$startdate = explode("/",$Startdate3);
		$enddate = explode("/",$Enddate3);
		echo 'since '.($startdate[0]-1911).' 年year '.$startdate[1].' 月month '.$startdate[2].' date日至until '.($enddate[0]-1911).' 年year '.$enddate[1].' 月month '.$enddate[2].' 日止date';
	}
	?>
    </td>
  </tr>
...
<div style="width:800px; font-size:18pt;">
  <center>中　　華　　民　　國A.D.　　<?php echo date(Y)-1911; ?>　　年year　　<?php echo date(m); ?>　　月month　　<?php echo date(d); ?>　　日date</center>
</div>
