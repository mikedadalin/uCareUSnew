...
$r = $db->fetch_assoc();
$url = "index.php?mod=rehabilitation&func=formview&pid=".$r['patientID'];
echo "<script type='text/javascript'>";
echo "window.location.href='".$url."'";
echo "</script>";
}else{
$db3 = new DB;
$db3->query("SELECT * FROM `nurseform02j` WHERE `Q20_1`='1'");
$db4 = new DB;
$db4->query("SELECT * FROM `nurseform31` WHERE `Q3_2`='1'");
$count=0;
?>
...
<div id="slider_scroll">
    ...
    <div id="slider_content">
      <div class="patlistbtn"><a href="index.php?mod=rehabilitation&func=formview&id=1" title="Wheelchair usage"><i class="fa fa-wheelchair fa-2x fa-fw"></i><br>Usage of Wheelchair</a></div>
      <div class="patlistbtn">
        ...
        if ($db3->num_rows()>0  || $db4->num_rows()>0) {
          ...
          for ($i3=0;$i3<$db3->num_rows();$i3++) {
            ...
            $db3b = new DB;
            $db3b->query("SELECT * FROM `socialform20_1` WHERE `HospNo`='".$r3['HospNo']."' AND `Qreplyto`='".$r3['date']."'");
            if ($db3b->num_rows()==0) {
              $count++;
              if ($i3>0) { echo '<br>'; }
              echo '<a href="index.php?mod=rehabilitation&func=formview&pid='.$pid.'&id=20" title="Pain notification">Pain notification<br>'.getBedID($pid).'<br>'.getPatientName($pid).'</a><br>-----------';
            }
          }
          echo '</div>';
        }
          ...
          for ($j=0;$j<$db1->num_rows();$j++) {
              ...
              if (count($arrAreaBed)==0 || in_array($r1['bed'],$arrAreaBed)) {
                  ...
                  echo '>
                    <td class="link1">
                      <center>
                      <a href="index.php?mod=rehabilitation&func=formview&pid='.$r['patientID'].'&query='.@$_GET['query'].'" title="Edit" onmouseover="showPatientPic('.$r['patientID'].');" onmouseout="hiddenPatientPic('.$r['patientID'].');"><i class="fa fa-magic fa-lg"></i></a>
                      </center>
                    </td>
                    <td>'.$r2b['areaName'].'</td>
                    <td>'.$r1['bed'].'</td>
